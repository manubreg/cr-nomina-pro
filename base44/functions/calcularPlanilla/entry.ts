  ...
  for (const { empleado_id, movs } of movimientosTemp) {
    ...
    for (const m of movs) {
      todosMovimientos.push({
        ...
        base_calculo: Number(m.base_calculo) || 0,
        planilla_id,
        planilla_detalle_id,
        empleado_id,
        concepto_id: planilla_id, // fallback válido como string
      });
    }
  }
